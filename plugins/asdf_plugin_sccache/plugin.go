...
var config = asdf.BinaryPluginConfig{
	...
	FileNameTemplate: "sccache-v{{.Version}}-{{.Arch}}-unknown-{{.Platform}}-musl.tar.gz",
	...
	HelpDescription: "sccache - Shared Compilation Cache",
	HelpLink:        "https://github.com/mozilla/sccache",
	ArchiveType:     "tar.gz",
}

// New creates a new sccache plugin instance.
func New() asdf.Plugin {
	return asdf.NewBinaryPlugin(&config)
}

// NewWithClient creates a new sccache plugin with a custom GitHub client.
