...
var config = asdf.BinaryPluginConfig{
	...
	FileNameTemplate:    "helm-v{{.Version}}-{{.Platform}}-{{.Arch}}.tar.gz",
	DownloadURLTemplate: "https://get.helm.sh/{{.FileName}}",
	HelpDescription:     "Helm - The Kubernetes Package Manager",
	HelpLink:            "https://github.com/helm/helm",
	ArchiveType:         "tar.gz",
}

// New creates a new Helm plugin instance.
func New() asdf.Plugin {
	return asdf.NewBinaryPlugin(&config)
}

// NewWithClient creates a new Helm plugin with a custom GitHub client.
