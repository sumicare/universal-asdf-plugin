...
var config = asdf.BinaryPluginConfig{
	...
	ArchiveType:         "tar.gz",
	VersionFilter:       `^\d+\.\d+\.\d+$`,
	UseReleases:         boolPtr(false),
}

// New creates a new protoc-gen-go-grpc plugin instance.
func New() asdf.Plugin {
	return asdf.NewBinaryPlugin(&config)
}

// NewWithClient creates a new protoc-gen-go-grpc plugin with a custom GitHub client.
