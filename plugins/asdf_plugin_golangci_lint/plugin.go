...
package asdf_plugin_golangci_lint
...
var config = asdf.BinaryPluginConfig{
	Name:       "golangci-lint",
	RepoOwner:  "golangci",
	RepoName:   "golangci-lint",
	BinaryName: "golangci-lint",

	FileNameTemplate: "golangci-lint-{{.Version}}-{{.Platform}}-{{.Arch}}.tar.gz",
	HelpDescription:  "golangci-lint - Fast linters runner for Go",
	HelpLink:         "https://github.com/golangci/golangci-lint",
	ArchiveType:      "tar.gz",
}

// New creates a new golangci-lint plugin instance.
func New() asdf.Plugin {
	return asdf.NewBinaryPlugin(&config)
}

// NewWithClient creates a new golangci-lint plugin with a custom GitHub client.
