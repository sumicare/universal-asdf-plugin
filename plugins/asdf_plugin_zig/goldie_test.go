...
func TestHelpGoldie(t *testing.T) {
	...
	goldieRecorder.Assert(t, "zig_help", []byte(strings.Join([]string{h.Overview, h.Deps, h.Config, h.Links}, "\n\n")))
}
...
func TestListBinPathsGoldie(t *testing.T) {
	p := New()
	goldieRecorder := goldie.New(t)
	goldieRecorder.Assert(t, "zig_bin_paths", []byte(p.ListBinPaths()))
}
...
func TestListLegacyFilenamesGoldie(t *testing.T) {
	p := New()
	goldieRecorder := goldie.New(t)
	goldieRecorder.Assert(t, "zig_legacy_filenames", []byte(strings.Join(p.ListLegacyFilenames(), "\n")))
}
...
func TestExecEnvGoldie(t *testing.T) {
	...
	goldieRecorder := goldie.New(t)
	goldieRecorder.Assert(t, "zig_exec_env", []byte(strings.Join(output, "\n")))
}
