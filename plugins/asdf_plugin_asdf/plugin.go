...
const (
	// repoOwner is the GitHub repository owner for asdf.
	repoOwner = "asdf-vm"
	// repoName is the GitHub repository name for asdf.
	repoName = "asdf"
)

// Plugin implements asdf plugin functionality for asdf itself.
type Plugin struct {
	*asdf.BinaryPlugin
}

// config defines the BinaryPluginConfig used by the asdf plugin.
//
...
var config = asdf.BinaryPluginConfig{
	...
	DownloadURLTemplate: "https://github.com/" + repoOwner + "/" + repoName +
...
// New creates a new asdf plugin instance.
func New() asdf.Plugin {
	return NewWithClient(github.NewClient())
}

// NewWithClient creates a new asdf plugin with a custom GitHub client.
func NewWithClient(client *github.Client) asdf.Plugin {
	bp := asdf.NewBinaryPlugin(&config)
	bp.WithGithubClient(client)

	return &Plugin{BinaryPlugin: bp}
}
...
func GetShellConfigInstructions(shell string) string {
	switch shell {
	case "bash":
