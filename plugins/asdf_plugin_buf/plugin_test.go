...
// pluginTestConfig returns the test configuration for the buf plugin.
...
var _ = Describe("Buf Plugin", func() {
	...
	testutil.DescribeMockOnlyListAll(cfg)
	testutil.DescribeMockOnlyLatestStable(cfg)
	testutil.DescribeMockOnlyDownload(cfg)
	testutil.DescribeMockOnlyInstall(cfg)
})
