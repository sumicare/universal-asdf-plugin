...
var config = asdf.BinaryPluginConfig{
	Name:       "velero",
	RepoOwner:  "vmware-tanzu",
	RepoName:   "velero",
	BinaryName: "velero",

	FileNameTemplate: "velero-v{{.Version}}-{{.Platform}}-{{.Arch}}.tar.gz",
	HelpDescription:  "Velero - Backup and migrate Kubernetes resources",
	HelpLink:         "https://velero.io/",
	ArchiveType:      "tar.gz",
	VersionFilter:    `^\d+\.\d+\.\d+$`,
}

// New creates a new Velero plugin instance.
func New() asdf.Plugin {
	return asdf.NewBinaryPlugin(&config)
}

// NewWithClient creates a new Velero plugin with a custom GitHub client.
