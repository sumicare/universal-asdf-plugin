...
var config = asdf.BinaryPluginConfig{
	Name:       "nerdctl",
	RepoOwner:  "containerd",
	RepoName:   "nerdctl",
	BinaryName: "nerdctl",

	FileNameTemplate: "nerdctl-{{.Version}}-{{.Platform}}-{{.Arch}}.tar.gz",
	HelpDescription:  "nerdctl - Docker-compatible CLI for containerd",
	HelpLink:         "https://github.com/containerd/nerdctl",
	ArchiveType:      "tar.gz",
}

// New creates a new nerdctl plugin instance.
func New() asdf.Plugin {
	return asdf.NewBinaryPlugin(&config)
}

// NewWithClient creates a new nerdctl plugin with a custom GitHub client.
