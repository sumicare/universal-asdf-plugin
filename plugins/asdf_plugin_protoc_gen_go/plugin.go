...
package asdf_plugin_protoc_gen_go
...
var config = asdf.BinaryPluginConfig{
	...
	RepoName:   "protobuf-go",
	BinaryName: "protoc-gen-go",

	FileNameTemplate:    "protoc-gen-go.v{{.Version}}.{{.Platform}}.{{.Arch}}.tar.gz",
	DownloadURLTemplate: "https://github.com/{{.RepoOwner}}/{{.RepoName}}/releases/download/v{{.Version}}/{{.FileName}}",
	HelpDescription:     "protoc-gen-go - Go support for Protocol Buffers",
	HelpLink:            "https://pkg.go.dev/google.golang.org/protobuf",
	ArchiveType:         "tar.gz",
	VersionFilter:       `^\d+\.\d+\.\d+$`,
}

// New creates a new protoc-gen-go plugin instance.
func New() asdf.Plugin {
	return asdf.NewBinaryPlugin(&config)
}

// NewWithClient creates a new protoc-gen-go plugin with a custom GitHub client.
