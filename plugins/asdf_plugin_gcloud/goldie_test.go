...
func TestHelpGoldie(t *testing.T) {
	...
	goldieRecorder.Assert(t, "gcloud_help", []byte(strings.Join([]string{h.Overview, h.Deps, h.Config, h.Links}, "\n\n")))
}
...
func TestListBinPathsGoldie(t *testing.T) {
	p := New()
	goldieRecorder := goldie.New(t)
	goldieRecorder.Assert(t, "gcloud_bin_paths", []byte(p.ListBinPaths()))
}
...
func TestListLegacyFilenamesGoldie(t *testing.T) {
	...
	goldieRecorder := goldie.New(t)
	goldieRecorder.Assert(t, "gcloud_legacy_filenames", []byte(output))
}
...
func TestExecEnvGoldie(t *testing.T) {
	...
	goldieRecorder := goldie.New(t)
	goldieRecorder.Assert(t, "gcloud_exec_env", []byte(strings.Join(output, "\n")))
}
...
func TestListAllGoldie(t *testing.T) {
	if asdf.IsOnline() {
		...
		goldieRecorder := goldie.New(t)
		goldieRecorder.Assert(t, "gcloud_list_all", []byte(strings.Join(versions, "\n")))

		return
	}

	testdataPath := testutil.GoldieTestDataPath(t)
	if !testutil.GoldieFileExists(testdataPath, "gcloud_list_all.golden") {
		t.Skip("gcloud_list_all.golden not found - run with ONLINE=1 to create")
	}
...
func TestLatestStableGoldie(t *testing.T) {
	if asdf.IsOnline() {
		...
		goldieRecorder := goldie.New(t)
		goldieRecorder.Assert(t, "gcloud_latest_stable", []byte(version))

		return
	}

	testdataPath := testutil.GoldieTestDataPath(t)
	if !testutil.GoldieFileExists(testdataPath, "gcloud_latest_stable.golden") {
		t.Skip("gcloud_latest_stable.golden not found - run with ONLINE=1 to create")
	}
